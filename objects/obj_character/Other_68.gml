if (async_load[? "size"] > 0) {
	...
	if (ds_map_find_value(response, "type") == msg_type.GET_PLAYER_STAT) {
		...
		if (pn == instance_player._id) {
			...
			instance_player.key.shot = ds_map_find_value(key, "shot");
			instance_player.key.discart = ds_map_find_value(key, "discart");
			instance_player.key.pickup = ds_map_find_value(key, "pickup");
